...
| Метод | Путь | Назначение |
|---|---|---|
| `GET` | `/api/engines` | Паспорта движков: подписи, описания, поддержка ударений, клонирования и профилей просодии, языки, объявленный откат, режимы работы и границы ручек (из этого строится UI) |

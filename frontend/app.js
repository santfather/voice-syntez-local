...
// проекта (project-io.js) и запись голоса с микрофона (voice-record.js). Связь
// двусторонняя: модули зовут перерисовку соседних вкладок через `loadVoices`,
// а события их элементов — отсюда.
import {
  cancelJob,
  setPreviewStatus,
  renderVoiceCards,
...
// таймлайн (timeline.js), экспорт/импорт проекта (project-io.js) и запись голоса
// с микрофона (voice-record.js). Списки совпадают с их `import`: модули связаны
// двусторонне, но значения читаются только внутри функций — на этапе оценки
// модуля цикл безопасен.
export {
  state,
  $,
  esc,

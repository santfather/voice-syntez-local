...
def _port_and_lock_check(start: int = 8000, tries: int = 20) -> tuple[list[dict], dict]:
    ...
    if held:
        ...
        # Файл остаётся намеренно: замок — это удержание flock, а не наличие файла,
        # и удалять его нельзя (окно гонки между unlink и close). Поэтому файл без
        # владельца — норма, а не предупреждение: он ничего не блокирует и не
        # мешает следующему запуску (см. `launcher.lock_is_held`).
        items.append(
            check(
                "замок",
                OK,
                "файл .voice_syntez.lock есть, но никем не удерживается (stale — безопасно)",
            )
        )
    else:

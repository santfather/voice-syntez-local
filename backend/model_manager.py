...
class ModelManager:
    ...
    def guard_delete(spec: ModelSpec) -> None:
        ...
        Выгрузки движка в этой фазе нет (она в фазе 10), поэтому текст честно
        говорит, что делать пользователю, а не обещает кнопку, которой нет.
        """
        if engine_in_use(spec.engine_id):
            raise ModelBusyError(
                f"Модель «{spec.label}» сейчас используется движком. Дождитесь "
                f"окончания задачи в очереди; если движок уже поднят и простаивает, "
                f"он останется в памяти до перезапуска приложения — выгрузка движков "
                f"появится в следующей фазе. Удаление файлов под работающей моделью "
                f"привело бы к падению синтеза."
            )

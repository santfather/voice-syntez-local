...
DEFAULT_SEC_PER_CHAR: dict[str, float] = {
    ...
    "xtts": 0.055,
    "xtts-banana": 0.07,
    # Qwen3-TTS — авторегрессивная языковая модель: на каждый кодовый кадр
    # (12 Гц) приходится шаг декодера, поэтому на знак текста она дороже
    # диффузионного F5. Значение — порядок величины между XTTS и её файнтюном:
    # до первого живого прогона это оценка, и её перепишут наблюдения.
    "qwen3-tts": 0.06,
    # Kokoro-ru — 82 млн параметров на CPU против GPU-диффузии и авторегрессии
    # выше. Опубликованный RTF файнтюна 0.102 на CPU — это время синтеза на
    # секунду аудио; в речи ~15 знаков на секунду, отсюда ~0.007 с/знак.
    "kokoro-ru": 0.007,
...
DEFAULT_COLD_START_SEC: dict[str, float] = {
    ...
    "xtts": 25.0,
    "xtts-banana": 32.0,
    # 1.7 ГБ параметров плюс отдельный декодер-токенизатор: холодный подъём
    # дороже XTTS. Тоже оценка до первого прогона.
    "qwen3-tts": 45.0,
    # Kokoro-ru поднимает не гигабайтные веса, а произносительный слой: словарь
    # репозитория (RUAccent) и espeak-data. Оценка до первого прогона.
    "kokoro-ru": 8.0,
    "": 20.0,
}

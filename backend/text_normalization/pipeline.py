...
0. Юникод приводится к NFC: текст из чужих источников приходит в разных формах,
   а шаблоны ниже написаны под композированную запись. Без этого шага
   декомпозированная «ё» («е» + U+0308) проходит мимо шага «ё» как обычная «е».
...
def _normalize(
    ...
    *,
    ...
    # Сначала каноническая композиция: текст из чужих источников приходит в разных
    # юникод-формах, а все шаблоны ниже — и особенно шаг «ё» — знают только
    # композированную запись. Декомпозированная «ё» («е» + U+0308) иначе прошла бы
    # мимо шага «ё» как обычная «е», и модель произнесла бы её неверно.
    # Сравнение в конце лога — с уже приведённым текстом: это не правка смысла,
    # а приведение записи, и в отчёте «до модели» ему делать нечего.

...
# Максимум одновременно загруженных TTS-движков (проактивный memory guard, задача
# «Проактивный memory guard»): на 18 ГБ два движка — уже 8–9 ГБ физического следа,
# третий уводит систему в swap. На 32+ ГБ можно поднять переменной окружения.
MAX_CONCURRENT_TTS_ENGINES = int(os.environ.get("VOICE_SYNTEZ_MAX_ENGINES", "2"))
# Минимальный запас свободной памяти системы (%), ниже которого загрузка нового
# движка отклоняется: это политика «не начинать, если места заведомо не хватит»,
# а не реакция на уже наступившую перегрузку (за неё отвечает порог ниже).
MIN_FREE_SYSTEM_PERCENT = int(os.environ.get("VOICE_SYNTEZ_MIN_FREE_PCT", "20"))
# Порог, выше которого система считается перегруженной. То же значение, что у
# `SYSTEM_MEM_THRESHOLD_PERCENT`, и намеренно не второй константой: «система
# перегружена» — одно понятие, и разойтись эти два порога не должны (см.
# memory_guard). Отличается он от MIN_FREE_SYSTEM_PERCENT: там запас, здесь факт.
SYSTEM_PRESSURE_PERCENT = SYSTEM_MEM_THRESHOLD_PERCENT
...
# Максимальная длина текста для preview (символов). Лимит рендера (MAX_TEXT_CHARS)
# для preview не годится: стадии считает та же синхронная функция, что и синтез, и
# 50 000 знаков заставили бы панель ждать минуты, не показав ничего полезного.
# 2 000 знаков — с большим запасом больше, чем нужно, чтобы разглядеть стадии.
MAX_PREVIEW_CHARS = int(os.environ.get("TTS_MAX_PREVIEW_CHARS", "2000"))

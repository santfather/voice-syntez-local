...
* `free_warm_engines` делает то же вне расписания: её зовут, когда синтез упал с
  нехваткой памяти Metal, и тёплые соседние движки мешают повтору.
...
def free_warm_engines(exclude: str | None = None) -> list[str]:
    """Принудительно возвращает память всех поднятых и свободных движков.

    Политика простоя здесь не спрашивается намеренно: функция зовётся, когда
    память Metal уже кончилась (аудит §11.4), и ждать пятнадцати минут простоя,
    чтобы выгрузить мешающий движок, бессмысленно. Поэтому вызывать её можно
    только по реальному отказу, а не «на всякий случай»: выгрузка стоит секунд
    загрузки весов на следующем синтезе.

    `exclude` — движок, ради которого память и освобождается: его синтез будут
    повторять, и выгружать его значило бы платить за загрузку дважды. Занятые
    движки пропускаются: `unload()` всё равно откажет (`EngineBusyError`).

    Ошибка выгрузки не поднимается наверх: цель — освободить память, и упавшая
    выгрузка одного движка не повод не трогать остальные.
    """
    unloaded: list[str] = []
    for engine_id, engine in created_engines().items():
        if engine_id == exclude or not engine.is_loaded or engine.active_synthesizes:
            continue
        try:
            engine.unload()
        except Exception as exc:  # noqa: BLE001 — принудительная уборка не повод падать
            logger.warning(
                "Движок %s: принудительная выгрузка не удалась (%s)", engine_id, exc
            )
            continue
        unloaded.append(engine_id)
        logger.info("Движок %s выгружен принудительно: не хватило памяти MPS", engine_id)
    return unloaded
